...
public class webFilter implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest arg0, HttpServletResponse arg1, Object arg2) throws Exception {
		...
		if(path.endsWith("/adinfo") || path.endsWith("/index.jsp") || path.endsWith("/login") || path.endsWith("/register")){
			return true;
		}else{
			if(map == null){
				log.debug("用户未登录");
				PrintWriter out = arg1.getWriter();
				out.write("<script type='text/javascript'>alert('please Login first!!!');window.top.location.href='"+arg0.getContextPath()+"';</script>");
				out.flush();
				out.close();
				return false;
			}else{
